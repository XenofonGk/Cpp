...
- Operator overloading for a natural-reading API: `cart += item` adds a product, `cart -= sku` removes one by ID, `item += 5` restocks a quantity, and calls can be chained (`cart += apple += banana`).
